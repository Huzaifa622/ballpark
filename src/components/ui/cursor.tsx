...
const Cursor = () => {
  useEffect(() => {
    // Initial setup: Set the balls' positions relative to the mouse.
    gsap.set(".ball", { xPercent: -50, yPercent: -50 });

    const targets = gsap.utils.toArray(".ball"); // Get all the balls

    // Event listener to handle mouse movement
    window.addEventListener("mousemove", (e) => {
      // Animation for each ball when mouse moves
      gsap.to(targets, {
